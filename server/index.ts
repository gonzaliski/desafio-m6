  ...
  roomCollection
    ...
    .then((snap) => {
      ...
      const gameRoomRef = rtdb.ref(
        "/rooms/" + rtdbId.rtdbRoomId + "/currentGame/gameData"
      );
      gameRoomRef.once("value").then(
        (snap) => {
          const snapData = snap.val();
          const listData = map(snapData) as any;
          const getNames = listData.map((p) => {
            return p.name;
          });
  ...
  roomCollection
    ...
    .then((snap) => {
      ...
      const playerRef = rtdb.ref(
        `/rooms/${rtdbId.rtdbRoomId}/currentGame/gameData/${userId}`
      );
      playerRef.update(
        {
          wins: wins,
